...
export default function Rapports() {
  ...
  const liste = useMemo(
    () => employes.filter((e) => service === "Tous services" || e.department === service),
    [service]
  );

  const stats = useMemo(() => {
    const arr = liste.map((e) => ({ e, p: productiviteEmploye(e.id), hp: historiquePresence(e.id) }));
    const n = arr.length || 1;
    const avg = (f) => Math.round(arr.reduce((s, x) => s + f(x), 0) / n);
    let pr = 0, re = 0, ab = 0, cg = 0;
    arr.forEach((x) => x.hp.forEach((d) => {
      if (d.statut === "Présent") pr++;
      else if (d.statut === "Retard") re++;
      else if (d.statut === "Absent") ab++;
      else if (d.statut === "Congé") cg++;
    }));
    // Taux de présence : congés exclus du dénominateur (même base que le donut).
    const presence = (jr) => {
      const base = jr.filter((d) => d.statut !== "Congé").length || 1;
      return Math.round((jr.filter((d) => d.statut === "Présent" || d.statut === "Retard").length / base) * 100);
    };
    const travailMin = avg((x) => parseHM(x.p.tempsMoyen));
    const inactMin = avg((x) => parseHM(x.p.inactivite));
    const serie = Array.from({ length: 7 }, (_, i) => avg((x) => x.p.serie[i] ?? 0));
    const classement = arr
      .map((x) => ({ id: x.e.id, name: x.e.name, fonction: x.e.fonction, score: x.p.score }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 6);
    return {
      n: arr.length,
      productivite: avg((x) => x.p.score),
      presence: avg((x) => presence(x.hp)),
      travailMin,
      ecranMin: Math.max(0, travailMin - inactMin),
      serie,
      jour: serie[6],
    ...
    <div className="space-y-5 pb-12">
      ...
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="ml-auto text-xs text-muted">{stats.n} agents</span>
        </div>
        <ol className="divide-y divide-border">
          {stats.classement.map((a, i) => (
